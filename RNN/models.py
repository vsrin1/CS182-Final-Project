...
def kaggle_model(num_epochs, tokenizer, training_reviews, test_reviews, training_stars, test_stars):
    ...
    """def custom_loss(y_true, y_pred):
        return K.dot(K.cast(y_true, dtype = tf.float32), y_pred) / 64
        #return K.log((1 - (K.sum(K.cast(K.equal(K.argmax(y_true), K.argmax(y_pred)), dtype = tf.int32)) / 64)))"""
    
    def custom_accuracy(y_true, y_pred):
        return K.sum(K.cast(K.equal(K.argmax(y_true), K.argmax(y_pred)), dtype = tf.int32)) / 64
    ...
    x = tf.keras.layers.Embedding(vocab_size, embedding_dim, embeddings_initializer = tf.keras.initializers.Constant(embeddings), trainable = True)(inp)
    ...
    outp = tf.keras.layers.Dense(5, activation = 'softmax')(conc)
...
def get_fit_samples(train_df, test_df, num, segmentation = .8):
    import random
    start_train = random.randint(0, len(train_df) - num)
    start_test = random.randint(0, len(test_df) - num)
    training_reviews = list(train_df["text"][start_train:start_train + int(num * segmentation)])
    training_stars = train_df[start_train:start_train + int(num * segmentation)][["stars_1.0", "stars_2.0", "stars_3.0", "stars_4.0", "stars_5.0"]].values
    test_reviews = list(test_df["text"])[start_test + int(num * segmentation):start_test + num]
    test_stars = test_df[start_test + int(num * segmentation):start_test + num][["stars_1.0", "stars_2.0", "stars_3.0", "stars_4.0", "stars_5.0"]].values
    return training_reviews, training_stars, test_reviews, test_stars

def model_extra_fit(model, num_epochs, training_reviews, test_reviews, training_stars, test_stars):
    import tensorflow as tf
    import numpy as np
    vocab_size = 5000
    embedding_dim = 200
...
def perturb_sample(sample, perturbations):
    import random
    out = sample.lower()
    for i in sample.lower().split():
        for j in perturbations.keys():
            if j in i:
                length = len(perturbations[j])
                out.replace(j, perturbations[j][random.randint(0, length - 1)])
    return out

def get_fit_samples_perturbed(train_df, test_df, num, segmentation = .8):
    import random
    perturbations = dict()
    file = open("perturbations.txt") 
    curr = None
    for i in file:
        if curr:
            perturbations[curr] = i.rstrip().split()
            curr = None
        else:
            curr = i.rstrip()
    start_train = random.randint(0, len(train_df) - num)
    start_test = random.randint(0, len(test_df) - num)
    training_reviews = list(train_df["text"][start_train:start_train + int(num * segmentation)])
    training_reviews = [perturb_sample(i, perturbations) for i in training_reviews]
    training_stars = train_df[start_train:start_train + int(num * segmentation)][["stars_1.0", "stars_2.0", "stars_3.0", "stars_4.0", "stars_5.0"]].values
    test_reviews = list(test_df["text"])[start_test + int(num * segmentation):start_test + num]
    test_stars = test_df[start_test + int(num * segmentation):start_test + num][["stars_1.0", "stars_2.0", "stars_3.0", "stars_4.0", "stars_5.0"]].values
    return training_reviews, training_stars, test_reviews, test_stars

def load_training_reviews(filename):
    import pandas as pd
    return list(pd.read_csv(filename)["0"])


def save_load_model(load, filenames, model = None, training_reviews = None):
    import tensorflow as tf
    import pandas as pd
    if load:
        model = tf.keras.models.load_model(filenames[0])
        training_reviews = load_training_reviews(filenames[1])
        return model, training_reviews
    else:
        model.save(filenames[0])
        pd.DataFrame(training_reviews).to_csv(filenames[1])
        return None, None

def evaluate_gen(training_reviews, test_reviews):
    import tensorflow as tf
    import numpy as np
    vocab_size = 5000
    embedding_dim = 200
    ...
    test_sequences = tokenizer.texts_to_sequences(test_reviews)
    test_padded = tf.keras.preprocessing.sequence.pad_sequences(test_sequences, maxlen = max_length, padding = padding_type, truncating = trunc_type)
    return training_padded, test_padded

def single_star_model(num_epochs, training_reviews, test_reviews, training_stars, test_stars):
    import tensorflow as tf
    import numpy as np
    vocab_size = 5000
    embedding_dim = 200
...
def single_star_pre(num_epochs, training_reviews, test_reviews, training_stars, test_stars):
    import tensorflow as tf
    import numpy as np
    vocab_size = 5000
    embedding_dim = 200
    ...
    #model.add(tf.keras.layers.concatenate([, tf.keras.layers.GlobalMaxPooling1D()]))
...
def tree_model(num_epochs, training_reviews, test_reviews, training_stars, test_stars):
    ...
    l1 = single_tree_node(vocab_size, embedding_dim, embeddings) # 1 if <= 3
    l21 = single_tree_node(vocab_size, embedding_dim, embeddings) # 1 if <= 2, 0 if 3
    l22 = single_tree_node(vocab_size, embedding_dim, embeddings) # 1 if 4, 0 if 5
    l31 = single_tree_node(vocab_size, embedding_dim, embeddings) # 1 if 1, 0 if 2
